...
public class jenatest2 extends Object {
    ...
    public static void main(String args[]) {
        ...
        // SPARQL query
        System.out.println("Paths between A and B :");
        dumpQueryResult(
                model,
                String.format(
                        "PREFIX goingto: <%s> SELECT * WHERE {?A ?firststop ?stopover . ?stopover ?secondstop ?B}",
                        uri("")));
    ...
    private static final void addTravelStuff(final Model model) {
        ...
        final Resource lhr = model.createResource(uri("India"))
                .addProperty(ba, london).addProperty(af, london);
        final Resource lgw = model.createResource(uri("Greece")).addProperty(
                ba, london);
